...
describe('<ConnectorTemplates>', () => {
  ...
  it('should show success notification on success and trigger reload of element templates', async () => {
    ...
    // when
    backend.receive('client:connector-templates-update-success');

    // then
    expect(displayNotificationSpy).to.have.been.calledWith({ type: 'success', title: 'Connector templates updated' });
  ...
  it('should show success notification with warnings on success with warnings', async () => {
    ...
    // when
    backend.receive('client:connector-templates-update-success', null, [ 'foo', 'bar' ]);

    // then
    expect(displayNotificationSpy).to.have.been.calledWithMatch({ type: 'warning', title: 'Connector templates updated with warnings' });
  ...
  it('should show error notification on error', async () => {
    ...
    // then
    expect(displayNotificationSpy).to.have.been.calledWithMatch({ type: 'error', title: 'Error updating connector templates' });
    expect(displayNotificationSpy.args[0][0].content).to.exist;
  });
