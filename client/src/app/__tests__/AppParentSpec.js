...
describe('<AppParent>', function () {
	...
	describe('workspace', function () {
		...
		it('should return promise on workspace save', async
			function () {
				// Given
				const workspace = new Workspace({
	...
	describe('trigger action', function () {
		...
		it('should handle quit aborted', async
			function () {
				// Given
				const backend = new Backend();
				const {
	...
	describe('client errors', function () {
		...
		it('should log client errors with string source attached', async
			function () {
				// given
				const backend = new Backend();
				const {
		...
		it('should log tab errors with file path attached', async
			function () {
				// given
				const tab = createTab({
					file: {
		...
		it('should log tab errors with filename attached if path is not present', async
			function () {
				// given
				const tab = createTab();
				const backend = new Backend();
		...
		it('should log tab errors with tab id attached if file is not present', async
			function () {
				// given
				const tab = createTab({
					file: undefined
		...
		it('should log client errors to backend', async
			function () {
				// given
				const backend = new Backend();
		...
		it('should log tab errors to backend with file path attached', async
			function () {
				// given
				const tab = createTab({
					file: {
		...
		it('should log tab errors to backend with filename attached if path is not present', async
			function () {
				// given
				const tab = createTab();
				const backend = new Backend();
		...
		it('should log tab errors to backend with tab id attached if file is not present', async
			function () {
				// given
				const tab = createTab({
					file: undefined
	...
	describe('client warnings', function () {
		it('should log client warnings', async
			function () {
				// given
				const backend = new Backend();
				const {
		...
		it('should log client warnings with string source attached', async
			function () {
				// given
				const backend = new Backend();
				const {
		...
		it('should log tab warnings with file path attached', async
			function () {
				// given
				const tab = createTab({
					file: {
		...
		it('should log tab warnings with filename attached if path is not present', async
			function () {
				// given
				const tab = createTab();
				const backend = new Backend();
		...
		it('should log tab warnings with tab id attached if file is not present', async
			function () {
				// given
				const tab = createTab({
					file: undefined
	...
	describe('plugins hint', function () {
		...
		it('should log plugins hint on error', async
			function () {
				// given
				Flags.init({
				...
				expect(app.state.logEntries[1]).to.eql({
					category: 'info',
					message: 'This error may be the result of a plug-in compatibility issue.'
				});
				expect(app.state.logEntries[2]).to.eql({
					category: 'info',
					message: 'Disable plug-ins (restarts the app)',
					action: appParent.togglePlugins
				});
			});
		it('should log plugins hint on relaunch', function () {
			...
			const {
				appParent
			} = createAppParent(mount);
			// then
			const app = appParent.getApp();
			expect(app.state.logEntries).to.eql([{
				category: 'info',
				message: 'Plugins are temporarily disabled.'
			}, {
				category: 'info',
				message: 'Enable plug-ins (restarts the app)',
				action: appParent.togglePlugins
			}]);
		});
...
function createAppParent(options = {}, mountFn = shallow) {
	...
	const globals = {...defaultGlobals, ...(options.globals || {})
	};
	...
	const tree = mountFn( < AppParentComponent globals = {
				globals
			}
			keyboardBindings = {
				keyboardBindings
