...
export class RPAEditor extends CachedComponent {
  ...
  handleListeners(editor, deregister) {
    ...
    editor.eventBus[method]('config.updated', this.saveConfig);
  ...
  saveConfig = (config) => {
    this.props.setConfig('rpa', { runtimeConfig: config });
    this.setCached({
  ...
  async createEditor() {
    ...
    const editor = RPACodeEditor({
      ...
      runtimeConfig: rpaConfig.runtimeConfig,
    ...
    this.setCached({
      ...
      lastRuntimeConfig: rpaConfig.runtimeConfig
    });
  ...
  async checkImport() {
    ...
    const rpaConfig = await this.props.getConfig('rpa', {});
    if (rpaConfig.runtimeConfig !== lastRuntimeConfig) {
      this.createEditor();
      return;
    }
  }
