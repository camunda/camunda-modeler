...
export default function TaskTestingTab(props) {
  ...
  const hasLintingErrors = linting && linting.some(({ category }) => category === 'error');

  const isConnectionConfigured = canConnectToCluster(connectionCheckResult) && !hasLintingErrors;
