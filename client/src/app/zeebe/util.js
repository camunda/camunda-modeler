...
function createOperateBaseUrl(endpoint) {
  ...
  // use explicit Operate URL for self-managed
...
export function getDeploymentUrls(tab, config, deploymentResult) {
  ...
  const operateBaseUrl = getOperateBaseUrl(endpoint);
  ...
  if (resourceType === RESOURCE_TYPES.BPMN) {
    ...
    const url = new URL(`${ operateBaseUrl }/processes`);
    ...
    return decisionDeployments.map(deployment => {
      ...
      const url = new URL(`${ operateBaseUrl }/decisions`);
...
export function getStartInstanceUrl(config, startInstanceResult) {
  ...
  return new URL(`${operateBaseUrl}/processes/${processInstanceKey}`).toString();
}
