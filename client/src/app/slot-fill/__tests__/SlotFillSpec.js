...
import { SlotFillRoot, Fill, Slot } from "..";
...
describe("slot-fill", function () {
  ...
  describe("<Fill>", function () {
    ...
    it("should register fill in strict mode", function () {
      const { getByTestId } = render(
        <StrictMode>
          <SlotFillRoot>
            <Fill slot="test">
              <div data-testid="test-fill-strict">Test Fill</div>
            </Fill>
            <Slot name="test" />
          </SlotFillRoot>
        </StrictMode>,
      ...
      expect(getByTestId("test-fill-strict")).to.exist;
    });

    it("should support hook-based fills in strict mode", function () {
      function HookFill() {
        ...
        return (
          <Fill slot="test" group="hook-fill">
            <button data-testid="hook-fill" onClick={handleToggle}>
              {open ? "open" : "closed"}
            </button>
          </Fill>
        );
      ...
      expect(getByTestId("hook-fill")).to.exist;
    });
    ...
    it("should unregister fill", function () {
      ...
      const { rerender, getByTestId, queryByTestId } = render(
        <TestComponent shouldRenderFill={true} />,
      );
          ...
          <SlotFillRoot>
            <Fill slot="foo">
              <div>
                ...
                <button onClick={() => setShowBar(false)} data-testid="toggle">
                  Toggle
                </button>
              </div>
            </Fill>
    ...
    it("should ignore unchanged fill re-registration", function () {
      let fillContext;
      ...
      expect(container.textContent).to.equal("B");
    });
  });
  ...
  describe("<Slot>", function () {
    ...
    it("should render fills in custom Component", function () {
      ...
      // then
      expect(CustomComponent).to.have.been.calledWithMatch({
        customProp: "foo",
      });
    });
