...
async function fetchAndUpdateTemplates(connectorTemplates, executionPlatformVersion, url) {
  ...
  if (!response.ok) {
    log.warn(`Failed to fetch templates (HTTP ${ response.status })`);
  ...
  for (let id in templatesByIdMetadata) {
    ...
    for (const templateMetadata of templatesMetadata) {
      ...
      if (!response.ok) {
        log.warn(`Failed to fetch template ${ id } version ${ templateMetadata.version } (HTTP ${ response.status })`);

        stats.error++;

        warnings.push(`Unable to fetch template ${ id } version ${ templateMetadata.version }`);
      ...
      try {
        ...
        log.warn(`Failed to fetch template ${ id } version ${ templateMetadata.version } from ${ ref }`, error);

        stats.error++;

        warnings.push(`Unable to fetch template ${ id } version ${ templateMetadata.version } from ${ ref }`);
...
function isTemplateCompatible(templateMetadata, executionPlatformVersion) {
  ...
  if (isPreCamunda8Range(engine[ 'camunda' ])) {
    log.info(`Template ${ templateMetadata.ref } is compatible with execution platform version ${ executionPlatformVersion } (pre-Camunda 8)`);

    return true;
  }

  if (engine[ 'camunda' ] && !semver.satisfies(executionPlatformVersion, engine[ 'camunda' ])) {
...
function isPreCamunda8Range(range) {
  const minVersion = semver.minVersion(range);

  return minVersion && minVersion.major === 0;
}