...
const { assign, find, isFunction, map, merge } = require("min-dash");
...
class MenuBuilder {
  ...
  getNewFileSubmenuTemplate() {
    const providedMenus = map(
      this.options.providers,
      (provider) => provider.newFileMenu
    ...
    const template = providedMenus.reduce((newFileMenus, current) => {
      return [...newFileMenus, ...current.map(mapMenuEntryTemplate)];
    }, []);
  ...
  appendMenuItem(builder, menuItem) {
    ...
    if (action) {
      menuItemOptions = {
        ...menuItemOptions,
        click: wrapActionInactiveInDevtools(() =>
          app.emit("menu:action", action, options)
        ),
      };
    }
    ...
    if (submenu) {
      menuItemOptions = {
        ...
        submenu: Menu.buildFromTemplate(
          menuItem.submenu.map(mapMenuEntryTemplate)
        ),
      };
    }
        ...
        try {
          ...
          menuItemDescriptor.submenu = Menu.buildFromTemplate(
            menuEntries.map((entry) => {
              const { accelerator, action, enabled, label, submenu, visible } =
                entry;
              ...
              return new MenuItem({
                ...
                click:
                  action &&
                  wrapActionInactiveInDevtools(wrapPluginAction(action, name)),
                submenu,
                visible,
              });
            })
  ...
  getHelpSubmenuTemplate() {
    const topPart = [
      {
        ...
        click: () =>
          browserOpen(
            "https://docs.camunda.org/manual/latest/modeler/camunda-modeler"
          ),
      },
      {
        label: "User Forum",
      ...
      getSeparatorTemplate(),
      {
        ...
        click: () =>
          browserOpen(
            "https://github.com/camunda/camunda-modeler/issues?q=is%3Aopen+is%3Aissue+label%3Aenhancement"
          ),
      },
      {
        label: "Report Issue",
    ...
    const bottomPart = [
      ...
      },
      {
        label: "What's new",
        click: () =>
          app.emit("menu:action", "emit-event", { type: "versionInfo.open" }),
      },
    ];
