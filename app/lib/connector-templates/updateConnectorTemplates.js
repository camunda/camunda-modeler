...
module.exports = async function updateConnectorTemplates(renderer, userPath) {
  try {
    const connectorTemplatesPath = path.join(userPath, 'resources/element-templates', connectorTemplatesFileName);
    ...
    const updatedConnectorTemplates = mergeConnectorTemplates(connectorTemplates, latestConnectorTemplates);

    fs.mkdirSync(path.dirname(connectorTemplatesPath), { recursive: true });

    fs.writeFileSync(connectorTemplatesPath, JSON.stringify(updatedConnectorTemplates, null, 2));

    renderer.send('client:connector-templates-update-success', warnings);
  } catch (error) {
...
async function fetchLatestConnectorTemplates() {
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch connector templates');
  }
  ...
  for (const item of items) {
    ...
    if (!response.ok) {
      log.warn('Failed to fetch connector template', item.name);

      warnings.push(`Unable to fetch connector template ${ item.name }`);
...
function mergeConnectorTemplates(connectorTemplates, latestConnectorTemplates) {
  for (const latestConnectorTemplate of latestConnectorTemplates) {
    const index = connectorTemplates.findIndex((connectorTemplate) => {
      return connectorTemplate.id === latestConnectorTemplate.id
        && connectorTemplate.version === latestConnectorTemplate.version;
    });

    if (index !== -1) {
      connectorTemplates[ index ] = latestConnectorTemplate;
    } else {
      connectorTemplates.push(latestConnectorTemplate);
    }
  }
