...
class ZeebeAPI {
   ...
   * @param { {
   *   endpoint: object,
   *   files: Array<{ path: string, contents?: string, type?: 'bpmn'|'dmn'|'form' }>,
   *   name?: string,
   *   tenantId?: string
   * } } parameters
   *
  ...
  async deploy(parameters) {
    ...
    files = files.map(file => {
      const { contents } = this._fs.readFile(file.path, { encoding: false });
    ...
    try {
      ...
      /** @type {Array<Resource>} */
      const resources = this._getResources(files, name, tenantId);
  ...
  /**
   * Get resources based on the provided files, name, and tenantId.
   *
   * @param {files: Array<{ path: string, contents?: string, type?: 'bpmn'|'dmn'|'form' }} files
   * @param {string} [name]
   * @param {string} [tenantId]
   *
   * @returns {Array<Resource>}
   */
  _getResources(files, name, tenantId) {
    return files.map(file => {
      const resource = {};

      if (files.length === 1 && name && name.length) {
        resource.name = this._fixResourceName(name, files[0].type);
      } else {
        resource.name = this._getResourceNameFromFile(file);
      }

      if (file.type === RESOURCE_TYPES.BPMN) {
        resource.process = file.contents;
      } else if (file.type === RESOURCE_TYPES.DMN) {
        resource.decision = file.contents;
      } else if (file.type === RESOURCE_TYPES.FORM) {
        resource.form = file.contents;
      } else {
      ...
      if (files.length === 1 && tenantId && tenantId.length) {
