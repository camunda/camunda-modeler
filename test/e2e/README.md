...
CI caches the download, keyed on the same content hash rather than on the
version (`c8run.js etag` resolves it), which saves ~25s per job. Only the
archive is cached, never the extracted distribution: Camunda 8 Run keeps its
database inside it (`camunda-data/h2db.mv.db`), so restoring that would carry
one run's deployed resources into every later run.
