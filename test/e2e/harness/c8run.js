...
// why both a local install and the CI cache are keyed on the artifact's ETag
// rather than on this string: the URL is mutable, so the version says nothing
// about the bytes behind it. Pin an exact patch (e.g. `8.9.19`, with the
...
/**
 * Print the pinned artifact's content hash, for the CI cache key. Resolving it
 * here keeps the platform's archive naming out of the workflow.
 *
 * @return {Promise<void>}
 */
